...
public partial class admin_ZhuoTeiList : System.Web.UI.Page
{
    /// <summary>
    /// 是 教育基础知识考试
    /// </summary>
    ...
    public void bangDing()
    {
        ...
        if (!string.IsNullOrWhiteSpace(_shichangCode))
        {
            using (var dbHelper = new DBHelper())
            {
                ...
                if (isPublicKaoShi)
                {
                    var lst0 = dbHelper.getZKZList().Where(p => p.shiChangCode == _shichangCode).ToList();
                    lst0.ForEach(t =>
                    {
                ...
                {
                    var lst0 = dbHelper.getZKZList().Where(p => p.shiChangCode1 == _shichangCode).ToList();
                    lst0.ForEach(t =>
                    {
                        ZhunKaoZhengListForAdmin item = new ZhunKaoZhengListForAdmin();
                        item.UserName = t.UserName;
